...
st.set_page_config(page_title="AI Course Creator", page_icon="🎓", layout="wide")
...
st.title("🎓 Autonomous AI Course Creator")
...
async def run_pipeline(topic, audience, c1, c2, c3):
    ...
    with c1:
        ...
        with st.status("🏗️ Architect is planning...", expanded=True) as status:
    ...
    # --- FINAL DOWNLOAD ---
    st.divider()
    st.success("🎉 Course Generation Complete! All modules processed.")
    
    with st.spinner("📄 Converting to Professional PDF..."):
        pdf_bytes = create_pdf(full_course_content)
    
    if pdf_bytes:
        st.download_button("📥 Download Professional Course (.pdf)", pdf_bytes, f"{topic}_Course.pdf", "application/pdf", type="primary")
    else:
        st.download_button("📥 Download as Markdown (.md)", full_course_content, f"{topic}_Course.md")
